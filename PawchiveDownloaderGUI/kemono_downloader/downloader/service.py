...
from .naming import post_folder_name, rename_images, sanitize_filename, unique_name
...
class DownloadService:
    def __init__(self, api: PlatformApi, include_other_attachments: bool = INCLUDE_OTHER_ATTACHMENTS):
        ...
        self._reserved_flat_names: set[str] = set()
    ...
    async def build_tasks(
        ...
        rename_images_enabled: bool = True,
        ...
        for post in posts:
            tasks.extend(
                await self.build_post_tasks(
                    post,
                    output_dir,
                    naming_mode,
                    rename_images_enabled=rename_images_enabled,
    ...
    async def build_post_tasks(
        ...
        if single_folder:
            content_name = sanitize_filename(f"{post.day}_{post.id}_content.txt")
        else:
            content_name = "content.txt"
        content_item = FileItem(url=post.url, name=content_name, kind="text", content="")
        if rename_images_enabled:
            items = rename_images([content_item, *files])
        else:
            used: set[str] = set()
            items = [FileItem(url=item.url, name=unique_name(item.name, used), kind=item.kind, content=item.content) for item in [content_item, *files]]
        ...
        post_dir = output_dir if single_folder else self._resolve_post_dir(output_dir, post, naming_mode)
        tasks: list[DownloadTask] = []
        for item in items:
            name = sanitize_filename(item.name)
            if single_folder:
                name = unique_name(name, self._reserved_flat_names)
            save_path = post_dir / name
            tasks.append(
                DownloadTask(
                    post=post,
                    file=item,
