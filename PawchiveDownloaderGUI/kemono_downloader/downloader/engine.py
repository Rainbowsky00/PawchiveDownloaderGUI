...
class DownloadEngine:
    ...
    async def _worker(
        ...
        while True:
            ...
            try:
                ...
                await self._run_task(session, task, on_event)
                if state_store:
                    state_store.mark_file(task, "finished")
                await self._mark_task(
                    task,
    ...
    async def _run_task(
        ...
        if task.file.kind == "text":
            ...
            byte_count = len(data) if isinstance(data, bytes) else len(str(data).encode("utf-8"))
            await emit_file_progress(on_event, task, 0, byte_count)
            ...
            await emit_file_progress(on_event, task, byte_count, byte_count)
            return
        ...
        for attempt in range(1, self.retries + 1):
            ...
            try:
                await self._download_file(session, task, on_event)
                return
            except Exception as exc:
                last_error = exc
    ...
    async def _download_file(
        ...
        async with session.get(task.file.url, headers=headers) as resp:
            ...
            total_size = response_total_size(resp, existing_size)
            downloaded = existing_size
            await emit_file_progress(on_event, task, downloaded, total_size)
...
def response_total_size(resp: aiohttp.ClientResponse, existing_size: int) -> int | None:
